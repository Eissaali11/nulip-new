...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/consume-moving-inventory", requireAuth, async (req, res) => {
    try {
      const user = (req as any).user;
      const { technicianId, n950, i900, rollPaper, stickers, mobilySim, stcSim } = req.body;
      ...
      // Get current moving inventory
      const movingInventory = await storage.getTechnicianInventory(technicianId);
      if (!movingInventory) {
        return res.status(404).json({ message: "Moving inventory not found" });
      }

      // Validate quantities
      if (n950 > movingInventory.n950Devices || i900 > movingInventory.i900Devices || 
          rollPaper > movingInventory.rollPaper || stickers > movingInventory.stickers ||
          mobilySim > movingInventory.mobilySim || stcSim > movingInventory.stcSim) {
        return res.status(400).json({ message: "Insufficient quantity in moving inventory" });
      }

      // Update moving inventory (decrease consumed amounts)
      await storage.updateTechnicianInventory(technicianId, {
        n950Devices: movingInventory.n950Devices - n950,
        i900Devices: movingInventory.i900Devices - i900,
        rollPaper: movingInventory.rollPaper - rollPaper,
      ...
      const fixedInventory = await storage.getTechnicianFixedInventory(technicianId);
      if (fixedInventory) {
        // Add consumed amounts back to fixed inventory
        await storage.updateTechnicianFixedInventory(technicianId, {
          n950Units: fixedInventory.n950Units + n950,
          i900Units: fixedInventory.i900Units + i900,
          rollPaperUnits: fixedInventory.rollPaperUnits + rollPaper,
          stickersUnits: fixedInventory.stickersUnits + stickers,
          mobilySimUnits: fixedInventory.mobilySimUnits + mobilySim,
          stcSimUnits: fixedInventory.stcSimUnits + stcSim,
        });
      }
      ...
      if (n950 > 0) {
        movements.push(storage.createStockMovement({
          ...
          packagingType: 'unit',
          quantity: n950,
          fromInventory: 'moving',
          toInventory: 'fixed',
          performedBy: user.id,
          reason: 'consumption',
          notes: 'Returned from moving to fixed inventory after consumption',
        }));
      }
      if (i900 > 0) {
        movements.push(storage.createStockMovement({
      ...
      res.status(500).json({ message: "Failed to consume moving inventory" });
    }
  });
